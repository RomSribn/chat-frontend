...
import { validateUsername, validateMessage, validateEmail } from '../validation';
...
describe('Validation Utils', () => {
  describe('validateUsername', () => {
    ...
    it('should reject usernames with invalid characters', () => {
      ...
      expect(result1.error).toBe('Username can only contain letters, numbers, hyphens, and underscores');

      const result2 = validateUsername('user name');
      expect(result2.isValid).toBe(false);
      expect(result2.error).toBe('Username can only contain letters, numbers, hyphens, and underscores');

      const result3 = validateUsername('user#name');
      expect(result3.isValid).toBe(false);
      expect(result3.error).toBe('Username can only contain letters, numbers, hyphens, and underscores');
    });
