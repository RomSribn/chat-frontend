...
describe('SocketService', () => {
  ...
  describe('sendMessage', () => {
    it('should emit send-message event with correct data', () => {
      ...
      expect(mockSocket.emit).toHaveBeenCalledWith('send-message', { username, content });
    });
    
    it('should connect automatically if not already connected', () => {
      socketService.sendMessage('user', 'message');
      
      expect(mockSocket.emit).toHaveBeenCalledWith('send-message', { username: 'user', content: 'message' });
    });
    ...
    it('should throw error if emit fails', () => {
      ...
      expect(() => socketService.sendMessage('user', 'message')).toThrow('Emit failed');
    });
  });
  ...
  describe('onNewMessage', () => {
    it('should add message listener and return unsubscribe function', () => {
      ...
      const newMessageHandler = mockSocket.on.mock.calls.find(
        (call: [string, (message: ChatMessage) => void]) => call[0] === 'new-message'
      )[1];
